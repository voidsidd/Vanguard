...
export class FastAPI extends CoreEl {
  constructor() {
    super();
    this._createEl();
  }
  ...
  private _createEl() {
    ...
    description.textContent = "Start from a clean, fresh FastAPI project.";
  ...
  private _create(name: string, path: string, main: boolean, venv: boolean) {
    window.ipc.invoke(
      "workbench.workspace.project.create.python.fastapi",
      name,
      path,
      main,
      venv
    ...
    window.ipc.once(
      "workbench.workspace.project.python.fastapi.complete",
      (_: any, path: string) => {
        _newProject._hide();
...
export const _pythonFastAPI = new FastAPI();
