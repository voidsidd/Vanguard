...
export class Flask extends CoreEl {
  constructor() {
    super();
    this._createEl();
  }
  ...
  private _createEl() {
    ...
    description.textContent = "Start from a clean, fresh Flask project.";
  ...
  private _create(name: string, path: string, main: boolean, venv: boolean) {
    window.ipc.invoke(
      "workbench.workspace.project.create.python.flask",
      name,
      path,
      main,
      venv
    ...
    window.ipc.once(
      "workbench.workspace.project.python.flask.complete",
      (_: any, path: string) => {
        _newProject._hide();
...
export const _pythonFlask = new Flask();
