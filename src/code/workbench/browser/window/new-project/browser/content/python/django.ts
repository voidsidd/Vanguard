...
export class Django extends CoreEl {
  constructor() {
    super();
    this._createEl();
  }
  ...
  private _createEl() {
    ...
    description.textContent = "Full-featured Django web application.";
    ...
    checkboxTitleVenv.textContent = "Create a virtual environment";
    ...
    const appNameForm = document.createElement("div");
    appNameForm.className = "form";

    const titleAppName = document.createElement("span");
    titleAppName.className = "title";
    titleAppName.textContent = "First app name";

    const inputWrapperAppName = document.createElement("div");
    inputWrapperAppName.className = "input-wrapper";

    const inputAppName = document.createElement("input");
    inputAppName.type = "text";
    inputAppName.placeholder = "myapp";

    inputWrapperAppName.appendChild(inputAppName);

    appNameForm.appendChild(titleAppName);
    appNameForm.appendChild(inputWrapperAppName);

    optionsForm.appendChild(appNameForm);
    ...
    create.addEventListener("click", () => {
      ...
      const appName = (inputAppName.value || "").trim();
      ...
      this._create(name, path, main, venv, appName);
    });
  ...
  private _create(
    ...
    window.ipc.once(
      "workbench.workspace.project.python.django.complete",
      (_: any, path: string) => {
        _newProject._hide();
...
export const _pythonDjango = new Django();
